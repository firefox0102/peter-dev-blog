---
title: Redux + ETL
tags: ["Redux", "Frontend Architechture", "Separtion of Concerns"]
date: 2020-07-03
...
In my last blog post about frontend patterns, I talked about the high level separation of concerns for each slice of the frontend. Today I want to talk a little more about the middle layer where most of our Redux code actually lives, the "Control Flow" layer, and give a little more information on my opinion of a very common discussion in the realm of application state.

## Cleaning Up Our App Store

While working on the Costello product at SalesLoft, I had a very interesting debate with my coworkers to try to figure out what our preference was for keeping data in the app redux store. While working on a new feature, I realized that we were fetching data from Elasticsearch, which gave us back a really deeply nested data object. We were storing the Elasticsearch hit straight in to our app redux store so we could access any piece of the data from anywhere in the app. This meant that whenever we might need a piece of the hit, it was probably already available in the application state.

However, after a while of working with this code, my spider senses started tingling. I felt like there were two major issues with our current approach that could be improved on.
...
Because our React code was directly accessing these objects, there were a LOT of points of failure that we needed to address whenever we realized that we had an edge case with a hit from Elasticsearch.

Check out this real example that extracts a name for a particular opportunity based on your CRM type. Remember, we had this kind of code everywhere in our React components.
...
The Second issue was this; by storing the Elasticsearch hit directly in state we were letting our backend technology dictate the way our frontend code was written. Now, to me this is actually even more important and is the kind of thing that keeps me up at night!

What if we decided to change where we fetched this data from, moving from Elasticsearch to a different API endpoint? By storing this in state and accessing these objects directly in the Presentational layer, we were coupling together our backend and our frontend in a way that could really hurt us in the future.

The above code example for `getLabel` lives directly inside of a React component. If we kept following that same pattern, imagine how many components we will have to change if we ever decide to stop using Elasticsearch.
...
Soounds like a pretty decent solution for our problem, right?
...
Pretty straight forward right? I consider that one of the key indicators of a good Architechtural pattern. If you make the **EASY** thing the **RIGHT** thing, your team will have a much easier time [falling in to the pit of success](https://blog.codinghorror.com/falling-into-the-pit-of-success/)!
...
Do you see how deeply nested the value of `the_data_we_actually_need` is nested in this Elasticsearch object? This might seem is a **REAL** example from a feaure I just finished (obviously simplified).

In this example, we create a TypeScript type for our Elasticsearch hit object shape called `IExampleESHit`, and a second type for our clean data state called `ITransformedData`. The point of these functions is just to transform from TypeA -> TypeB, and there's not much more to it!

Here's what our `IExampleESHit` TypeScript interface looks like. The important part here is that if any of these values might cause issues because they're not always returned from the API, you can easily add that logic to this TypeScript type and now there are no surprises!
...
Althought this concept seems really simple, it's important to remember how impactful this can be on a frontend project. By simply adding a Transformation layer between your API responses and your App Store, you could gain:
